...
export default function PostHistory({ showToast }) {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`/.netlify/functions/get-posts${qs}`);
    ...
    <div className="history fade-in">
      ...
      {posts.length === 0 ? (
        ...
        <div className="post-list">
          {posts.map(post => {
            const st = STATUS_MAP[post.status] || { label: post.status, cls: '' };
              ...
              <div key={post.id} className="post-row">
                ...
                <div className="post-body">
                  ...
                  <div className="post-meta">
                    ...
                    {post.error_message && (
                      <span className="text-red text-sm" title={post.error_message}>Error ⚠</span>
                    )}
                    {post.retry_count > 0 && (
                      <span className="text-dim">{post.retry_count} retries</span>
                    )}
                  </div>
                </div>
